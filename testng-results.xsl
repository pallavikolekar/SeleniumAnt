                ...
                <td nowrap="true">
	                 <xsl:choose>
						 <xsl:when test="./exception">
						   <a href="./FailedScreenShots/{@name}/screenshot-{@name}.png"><xsl:value-of select="concat(@name, '(', testng:trim(testng:concatParams(./params/param)), ')')"></xsl:value-of></a>
						 </xsl:when>
						 <xsl:otherwise>
	          			  <a href="">---No Screenshot---</a>
	        			</xsl:otherwise>
	        		</xsl:choose>
                </td>
            </tr>
